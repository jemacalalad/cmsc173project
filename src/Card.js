...
function Card({subject}){
    ...
    return(
        ...
        <div className={`m-5 flex flex-col bg-gradient-to-r from-${color}-200 to-${color2}-200 rounded-xl w-72`}>
            ...
            <button className={`my-5 p-3 font-bold text-white place-self-center w-max bg-${color}-600 hover:bg-${color}-400 active:bg-${color}-800 rounded-xl`} onClick={() => {console.log("Clicked")}}>View</button>
        </div>
    );
}
